...
router.post('/add-product', (req, res, next) => { //app.post or app.get limits the middleware to only trigger if the path is correct and the request is POST or GET
    ...
    products.push({title: req.body.title, price: req.body.price, description: req.body.description, rating: req.body.rating}); //edit here to add price,description, and rating
    res.redirect('/');
});
